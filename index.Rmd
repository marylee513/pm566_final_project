...
## Data Analysis and Visualization of Primary Research Interest (Obesity)
## First Step
...
fig2 <- vars %>%
  ...
  labs(title = "Fig2: BMI by UPF Quartiles", x = "BMI", fill = "UPF Quartile")
...
The scatterplot Figure1 shows the association between UPF% and BMI. The plot does not show a trend of UPF% on BMI. 
The histogram Figure2 BMI by UPF% shows that subjects who consumed 0-25% UPF had a pattern with right skewed, while subjects who consumed greater than 25% UPF showed no specific pattern. 
The barchart Figure3 shows in another way that most subjects consumed low amount of UPFs were either overweight or obese. 
